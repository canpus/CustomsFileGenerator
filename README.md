...
> **版本**: 6.0.1-a | **状态**: 阶段 9 完成 — GUI 界面就绪（3 文件模式）
...
1. **新建单据**：填写订单信息（发票号、合同号、客户信息等）→ 点击"下一步"
2. **编辑商品明细**：左侧树状结构新增托盘 → 纸箱 → 商品，右键克隆/删除节点
3. **一键生成**：点击"生成报关资料" → 进度条 → 输出目录查看生成文件
4. **模板管理**：保存/加载/删除订单模板，支持关键词搜索
5. **数据导入**：从 Excel 文件导入订单，或从数据库加载已保存模板
...
| 版本 | 日期 | 变更说明 |
|------|------|---------|
| 6.0.1-a | 2026-05-26 | 阶段 9 完成：GUI 界面（ttkbootstrap Flatly 主题）— 主应用窗口（DPI 感知 + 字体回退）、订单信息录入页（4 大区块 + Excel 导入回填）、托盘-纸箱-商品树状编辑器（三级结构 + 右键菜单克隆/删除）、表格导入页（Excel 文件 + 数据库模板）、模板管理页（保存/加载/删除 + 搜索）、生成与预览页（3 文件模式 + 进度条 + 诊断包导出）、`main.py --gui` 入口 |

...
def scan_invoice_template(ws: Worksheet, rules: dict | None = None) -> AnchorResult:
    ...
    col_keywords: list[str] = header_anchor.get("col_keywords", ["Specification", "Unit", "Qty"])
    ...
    if result.summary_rows:
        ...
        result.errors.append("[错误]: 发票模板汇总行扫描失败，未找到'Total'/'SAY'等关键词")

    if not result.is_valid:
        result.errors.append("[错误]: 发票模板锚点扫描不完整，请确认模板是否与原厂一致")
